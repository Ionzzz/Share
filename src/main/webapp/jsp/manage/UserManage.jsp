    ...
    <script>
        function changestatus(id,status){
            if(confirm("确认执行此操作？") == true){
                $.ajax({
                    url:"${pageContext.request.contextPath }/mngUser/changeStatus.action",
                    data:{
                        "id":id,
                        "status":status,
                    },
...
//                        "dataType":"jsonp",
                ...
                "columns":[
                    ...
                    { "data": "userId","render": function ( data, type, row, meta ) {
                        if(row.userstatus == 0){
                            return '<button onclick="changestatus('+data+','+row.userstatus+')" >解禁</button>';
                        }else{
                            return '<button onclick="changestatus('+data+','+row.userstatus+')" >禁言</button>';
                        }
                    }},
                ]
            });
